...
async function checkBackendConnection() {
    ...
    try {
        ...
        if (health.status === 'ok' && health.mysql_connected) {
            ...
            if (badge) {
                badge.innerText = 'Servidor: Conectado (SQLite Local)';
                badge.className = 'badge badge-success mb-3 text-center';
                // Reset styling in case it was modified by offline fallback
            ...
            setOfflineUI(badge, warning, health.mysql_error || 'Base de datos desconectada');
        }
    } catch (e) {
...
function setOfflineUI(badge, warning, reason) {
    ...
    if (badge) {
        badge.innerText = 'Modo: Local (Navegador)';
        badge.className = 'badge mb-3 text-center';
    ...
    if (warning) {
        ...
        warning.className = 'mt-2 text-info';
        warning.style.color = 'var(--accent-secondary)';
        warning.innerHTML = `⚡ <strong>Modo Autónomo Activo:</strong> El backend no está corriendo, por lo que usaremos el motor de simulación y el historial local de tu navegador (localStorage). ¡Todas las funciones CRUD e informes están 100% operativos!`;
    }
}
...
async function runExperiment() {
    ...
    try {
        if (isBackendOnline) {
            // --- MODO ONLINE (FLASK + SQLITE) ---
            const response = await SimulatorAPI.runExperiment(scenario, quantum, expName);
            currentExperimentData = response;
            statusBox.innerHTML = '<p style="color:var(--accent-success); font-weight:bold;">✓ Experimento guardado en SQLite local con éxito.</p>';
            loadHistoryList(); // Recargar historial automáticamente
        } else {
            // --- MODO OFFLINE (FALLBACK LOCAL CON PERSISTENCIA localStorage) ---
            ...
            // Guardar en LocalStorage y estructurar el experimento
            const saveRes = LocalBrowserStorage.saveExperiment(scenario, quantum, expName, results);
            currentExperimentData = saveRes;
            
            statusBox.innerHTML = '<p style="color:var(--accent-success); font-weight:bold;">✓ Experimento guardado en el navegador con éxito.</p>';
            loadHistoryList(); // Recargar historial automáticamente
        }
...
async function loadHistoryList() {
    ...
    try {
        ...
        if (experiments.length === 0) {
            container.innerHTML = '<p class="text-muted text-center py-4">No hay experimentos registrados en la base de datos.</p>';
            return;
        }
